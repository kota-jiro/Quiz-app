...
export const TeacherRegister = () => {
  ...
  const signupAPI = async (newPost) => {
    try {
      await axios.post(
        "https://apex.oracle.com/pls/apex/josh_mp_workspace/teacher/teacher",
        newPost
      );
      return true;
    } catch (err) {
  ...
  const TeacherSignup = async () => {
    ...
    status && navigate("/teacherlogin"); // after creating redirect to login page.
  };
      ...
      <div className="student-r-container">
        <div className="student-r-wrapper">
          ...
          <div className="student-r-button">
            <button onClick={TeacherSignup}>Confirm Register</button>
          </div>
